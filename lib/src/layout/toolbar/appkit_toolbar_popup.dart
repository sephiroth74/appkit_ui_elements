...
enum AppKitToolbarPopupPosition {
...
enum AppKitToolbarPopupPlacement {
...
/// {@endtemplate}
class AppKitToolbarPopup<T> extends StatefulWidget {
  /// {@macro toolbarPopup}
  const AppKitToolbarPopup({
    super.key,
    required this.child,
    required this.content,
    this.verticalOffset = 0,
    this.horizontalOffset = 0,
    this.placement = AppKitToolbarPopupPlacement.center,
    this.position = AppKitToolbarPopupPosition.above,
  });
  ...
  ///
  /// Defaults to [AppKitToolbarPopupPlacement.center].
  final AppKitToolbarPopupPlacement placement;

  /// The position of the popup.
  ///
  /// Defaults to [AppKitToolbarPopupPosition.above].
  final AppKitToolbarPopupPosition position;

  @override
  AppKitToolbarPopupState<T> createState() => AppKitToolbarPopupState<T>();
}

class AppKitToolbarPopupState<T> extends State<AppKitToolbarPopup<T>> {
  ...
  Future<void> openPopup() {
    ...
    final Offset directionalityTarget = () {
      switch (widget.placement) {
        case AppKitToolbarPopupPlacement.start:
          if (directionality == TextDirection.ltr) {
            return leftTarget;
          } else {
            return rightTarget;
          }
        case AppKitToolbarPopupPlacement.end:
          if (directionality == TextDirection.ltr) {
            return rightTarget;
          } else {
            return leftTarget;
          }
        case AppKitToolbarPopupPlacement.center:
          return centerTarget;
      }
    }();

    // The placement according to the current directionality.
    final AppKitToolbarPopupPlacement directionalityPlacement = () {
      switch (widget.placement) {
        case AppKitToolbarPopupPlacement.start:
          if (directionality == TextDirection.rtl) {
            return AppKitToolbarPopupPlacement.end;
          }
          continue next;
        case AppKitToolbarPopupPlacement.end:
          if (directionality == TextDirection.rtl) {
            return AppKitToolbarPopupPlacement.start;
          }
          continue next;
        next:
        default:
...
class _ToolbarPopupMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    final defaultOffset = position == AppKitToolbarPopupPosition.side
        ? horizontalPositionDependentBox(
            ...
            preferLeft: placement == AppKitToolbarPopupPlacement.end,
          )
        : positionDependentBox(
            ...
            target: target,
            verticalOffset: verticalOffset,
            preferBelow: position == AppKitToolbarPopupPosition.below,
            margin: horizontalOffset,
          );
    if (position == AppKitToolbarPopupPosition.side) {
