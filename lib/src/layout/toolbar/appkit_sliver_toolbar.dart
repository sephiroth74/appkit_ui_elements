...
/// Defines the height of a regular-sized [AppKitSliverToolBar]
const _kToolbarHeight = 52.0;

/// Defines the width of the [AppKitSliverToolBar]'s title.
...
/// {@endtemplate}
class AppKitSliverToolBar extends StatefulWidget with Diagnosticable {
  /// {@macro sliverToolBar}
  const AppKitSliverToolBar({
    super.key,
  ...
  /// Whether this [AppKitSliverToolBar] is allowed to perform wallpaper tinting
  /// overrides.
  ///
  /// This property is supposed to be set to true when this [AppKitSliverToolBar] is
  ...
  Widget build(
    ...
    if (isScrolledUnder) {
      effectiveDecoration = decoration?.copyWith(
          ...
          BoxDecoration(
            color: AppKitTheme.of(context).canvasColor.withOpacity(opacity),
          );
    }
