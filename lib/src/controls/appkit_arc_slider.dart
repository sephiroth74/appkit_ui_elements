...
class _AppKitArcSliderState extends State<AppKitArcSlider> {
  ...
  Offset get center => Offset((size / 2) + thumbRadius, (size / 2) + thumbRadius);
  ...
  _thumbPosition() {
    ...
    final sweepAngle = value * (360 - this.sweepAngle * 2) * (pi / 180) + (this.sweepAngle * pi / 180);
    return Offset(center.dx + (size / 2) * cos(sweepAngle - pi / 2), center.dy + (size / 2) * sin(sweepAngle - pi / 2));
  }
    ...
    return Semantics(
      ...
      child: Builder(
        builder: (context) {
          ...
          if (thumbRadius == 0) {
            thumbRadius = widget.thumbRadius ?? (sliderTheme.continuousThumbSize / 3);
          }
          ...
          final accentColor = isMainWindow
              ? (widget.progressColor ?? sliderTheme.sliderColor ?? theme.activeColor)
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: CustomPaint(
                      painter: _TrackPainter(
                        color: trackColor.withValues(alpha: trackColor.a * enabledFactor),
                  ...
                  Positioned(
                    ...
                    child: CustomPaint(
                      painter: _ProgressPainter(
                        color: accentColor.withValues(alpha: accentColor.a * enabledFactor),
                        value: value,
                        strokeWidth: widget.progressWidth ?? sliderTheme.trackHeight,
                        sweepAngle: sweepAngle,
                      ),
                    ),
                  ),
                  ...
                  Positioned(
                    ...
                    child: AppKitCircularSliderThumb(
                      ...
                      foregroundColor: (enabled && _panStarted) ? AppKitColors.fills.opaque.tertiary.color : null,
                    ),
                  ),
                ],
              ),
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(ObjectFlagProperty<ValueChanged<double>>.has('onChanged', widget.onChanged));
  }
}
...
class _ProgressPainter extends CustomPainter {
  ...
  _ProgressPainter({required this.color, required this.value, required this.strokeWidth, required this.sweepAngle});
...
class _TrackPainter extends CustomPainter {
  ...
  _TrackPainter({required this.color, required this.strokeWidth, required this.sweepAngle});
