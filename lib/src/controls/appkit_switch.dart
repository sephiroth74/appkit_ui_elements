...
class _AppKitSwitchState extends State<AppKitSwitch> with TickerProviderStateMixin {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(FlagProperty('checked', value: checked, ifTrue: 'checked', ifFalse: 'unchecked', showName: true));
    properties.add(FlagProperty('enabled', value: enabled, ifTrue: 'enabled', ifFalse: 'disabled', showName: true));
    properties.add(
      FlagProperty(
        'isInteractive',
        value: isInteractive,
    ...
    properties.add(DiagnosticsProperty<Offset>('dragStartPosition', dragStartPosition));
    properties.add(DiagnosticsProperty<ValueChanged<bool>?>('onChanged', widget.onChanged));
  ...
  void _handleTapDown(Offset localPosition) {
    setState(() {
      ...
      if (checked) {
        dragStartedOnHandle = localPosition.dx > ((_size.width - _handleSize - _handlePadding));
      } else {
        dragStartedOnHandle = localPosition.dx < ((_handleSize + _handlePadding));
      }
      mouseIsDown = true;
      isInteractive = true;
    });
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    if (dragStartedOnHandle) {
      if (details.localPosition.dx < ((_size.width) / 2)) {
        if (!positionController.isAnimating || positionController.status != AnimationStatus.reverse) {
          _animateTo(false);
        }
      } else {
        if (!positionController.isAnimating || positionController.status != AnimationStatus.forward) {
          _animateTo(true);
        }
      }
    } else {
          ...
          child: Semantics(
            ...
            child: Builder(
              builder: (context) {
                ...
                final uncheckedColor = enabled ? switchTheme.uncheckedColor : switchTheme.uncheckedColorDisabled;
                final checkedColor = enabled ? (widget.color ?? switchTheme.checkedColor) : uncheckedColor;
                ...
                final containerBackgroundColor = enabled
                    ? AppKitDynamicColor.resolve(context, AppKitColors.fills.opaque.secondary).withValues(
                        alpha:
                            AppKitDynamicColor.resolve(context, AppKitColors.fills.opaque.secondary).a *
                            (1.0 - animationValue),
                      )
                    : AppKitColors.fills.opaque.primary.color.withValues(alpha: 0.04);
                ...
                if (enabled) {
                  ...
                  final checkedInnerShadowColor = hsvColor.withLightness(hsvColor.lightness / 1.05).toColor();
                  final uncheckedInnerShadowColor = Colors.black.withValues(alpha: 0.2);
                  innerShadowColor = Color.lerp(uncheckedInnerShadowColor, checkedInnerShadowColor, animationValue)!;
                } else {
                ...
                return MouseRegion(
                  ...
                  child: GestureDetector(
                    ...
                    onTapDown: enabled ? (details) => _handleTapDown(details.localPosition) : null,
                    ...
                    onHorizontalDragDown: enabled ? (details) => _handleTapDown(details.localPosition) : null,
                    dragStartBehavior: DragStartBehavior.start,
                    child: SizedBox(
                      width: _size.width,
                      height: _size.height,
                      child: Container(
                        foregroundDecoration: enabled && statusController.value > 0.0
                            ? BoxDecoration(
                                color: Color.lerp(
                                  theme.controlBackgroundPressedColor.withValues(alpha: 0.0),
                          ...
                          child: DecoratedBox(
                            decoration: BoxDecoration(
                              ...
                              gradient: enabled
                                  ? LinearGradient(
                                      ...
                                      colors: [
                                        Color.fromRGBO(102, 102, 102, 0.3 * animationValue),
                            ...
                            child: Padding(
                              ...
                              child: Stack(
                                children: [
                                  Positioned(
                                    left: animationValue * ((_size.width - _handleSize - _handlePadding)),
                                    child: SizedBox(
                                      width: _handleSize,
                                      height: _handleSize,
                                      child: Container(
                                        decoration: BoxDecoration(
                                          color: Colors.black.withValues(alpha: 0.08 * enableFactor),
                                        ...
                                        child: DecoratedBox(
                                          decoration: BoxDecoration(
                                            ...
                                            boxShadow: [
                                              BoxShadow(
                                                color: Colors.black.withValues(alpha: enabled ? 0.12 : 0.02),
